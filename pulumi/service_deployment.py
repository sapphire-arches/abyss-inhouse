...
class ServiceDeployment(ComponentResource):
    ...
    def __init__(self,
        ...
        container = ContainerArgs(
            name=name,
            image=image,
            resources=resources or ResourceRequirementsArgs(requests={
                "cpu": "100m",
                "memory": "100Mi"
            }, ),
            ports=[ContainerPortArgs(container_port=p)
        ...
        self.service = Service(
            ...
            spec=ServiceSpecArgs(
                ports=[ServicePortArgs(port=p, target_port=p)
                       for p in ports] if ports else None,
        ...
        if allocate_ip_address:
            if is_minikube:
                self.ip_address = self.service.spec.apply(
                    lambda s: s.cluster_ip)
            else:
